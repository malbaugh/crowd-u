...
class ClosedContestsByOwner(Resource):
  def get(self,username):
    db_session = DB_SESSION()

    challenge_objects = db_session.query(ContestTable).filter(ContestTable.owner==username).filter((ContestTable.submit_date < datetime.now()) | (ContestTable.completed == True)).all()
