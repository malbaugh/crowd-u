...
class ContractsByOwner(Resource):
  def get(self,username):
    db_session = DB_SESSION()

    challenge_objects = db_session.query(ContractTable).filter((ContractTable.owner==username) & (ContractTable.submit_date >= datetime.now()) & (ContractTable.completed == False)).all()
