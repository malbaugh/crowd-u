...
class ChallengesUserIsRegisteredFor(Resource):
  def get(self,uid):
    db_session = DB_SESSION()
    
    registered_user_objects = db_session.query(ChallengeRegistrationTable).filter_by(user_id=uid).all()
    ...
    if ((user.id == int(uid)) and (check_password_hash(user.password, password))):
      ...
      for registered_user in registered_user_objects:
        challenge_id = registered_user.challenge_id
